...
class _TravelItem extends StatelessWidget {
      ...
      child: GestureDetector(
        onTap: () {
          if(item.article.urls != null && item.article.urls.length > 0) {
            NavigatorUtils.push(context, WebView(url: item.article.urls[0].h5Url, title: '详情',));
          }
        },
        child: Card(
          child: PhysicalModel(
  ...
  _itemImage() {
    return Stack(
      children: <Widget>[
        (item.article.images[0]?.dynamicUrl) == null ? Container() : Image.network(item.article.images[0]?.dynamicUrl),
        Positioned(
          bottom: 8,
          left: 8,
          child: Container(
            ...
            child: Row(
              children: <Widget>[
                Padding(
                  ...
                  child: Icon(
                    Icons.location_on, color: Colors.white, size: 13,
                  ),
                ),
                LimitedBox(
                  maxWidth: 100,
  ...
  String _poiName() {
    return item.article.pois == null || item.article.pois.length == 0
        ? '未知' : item.article.pois[0]?.poiName ?? '未知';
  }
          ...
          Row(
            children: <Widget>[
              PhysicalModel(
                ...
                child: (item.article.author?.coverImage?.dynamicUrl) == null ? Container() : Image.network(
